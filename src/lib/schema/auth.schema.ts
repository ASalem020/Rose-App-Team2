...
export const registerSchema = (t: Translations) => {
  ...
  return z.object({
    name: z.object({
      firstName: z.string(t("validation.name.type")).nonempty(t("validation.name.first-name-requested")),
      lastName: z.string(t("validation.name.type")).nonempty(t("validation.name.last-name-requested"))
    }),
    ...
    .regex(/(?=.*?[0-9])/, t("validation.password.numbers"))
    .regex(
      /(?=.*?[#?!@$%^&*-])/,
