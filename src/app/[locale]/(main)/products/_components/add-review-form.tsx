...
import {
    addReviewSchema,
    AddReviewFormData,
} from '@/app/[locale]/(main)/products/_validations/review.validation';
...
export default function AddReviewForm({ productId }: AddReviewFormProps) {
     ...
     * Expected URL format: /products/[productId]
     */
    const getProductIdFromUrl = (): string => {
        // TODO: Use useParams() or router.query when dynamic routing is implemented
        // const params = useParams();
        // return params.productId as string;

        // Temporary: Use hardcoded test ID
    ...
    const onSubmit = (data: AddReviewFormData) => {
        ...
        // Get product ID from URL (currently using test ID)
                                    ...
                                    {userRating >= star ? (
                                        ...
                                        <div className="relative">
                                            ...
                                            <StarHalf className={cn(
                                                "absolute inset-0 h-6 w-6 fill-yellow-400 text-yellow-400",
                                                isRtl && "-scale-x-100"
                                            )} />
                                        </div>
                                    ) : (
